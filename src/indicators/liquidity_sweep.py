...
def detect_liquidity_sweep(
    ...
    try:
        ...
        post_session_df = df[df.index.time >= start_time_obj]  # type: ignore[union-attr]
    ...
    for i, (idx, row) in enumerate(post_session_df.iterrows()):
        ...
        if high >= sweep_high_level:
            ...
            if volume_median is not None:
                vol_at_bar = volume_median.loc[idx] if idx in volume_median.index else None  # type: ignore[arg-type]
            ...
            return SweepInfo(
                ...
                direction="bullish",
                sweep_price=high,
                sweep_bar_index=int(df.index.get_loc(idx)),  # type: ignore[arg-type]
                sweep_time=idx,  # type: ignore[arg-type]
        ...
        if low <= sweep_low_level:
            ...
            logger.debug(
                f"Bearish sweep detected at {idx}: low={low:.4f}, sweep_level={sweep_low_level:.4f}"
            )
...
def validate_sweep_volume(
    df: pd.DataFrame, sweep_bar_index: int, lookback_bars: int = 20, threshold_mult: float = 1.0
) -> bool:
    """
    ...
    median_volume = float(volume_slice.median())
    sweep_volume = float(df.iloc[sweep_bar_index]["Volume"])

    return sweep_volume >= (median_volume * threshold_mult)


def detect_sweep_reversal(df: pd.DataFrame, sweep_info: SweepInfo, lookback: int = 3) -> bool:
    """
...
def get_sweep_statistics(
    ...
    for date in asia_highs.index:
        try:
            ...
            atr_value = (
                atr_series.loc[day_start] if day_start in atr_series.index else atr_series.iloc[-1]
            )
