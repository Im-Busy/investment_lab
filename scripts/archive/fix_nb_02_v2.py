...
for i, cell in enumerate(nb["cells"]):
    if cell["cell_type"] == "code":
        ...
        if "Try to load 5-minute data" in source and "data_config" in source:
            cell["source"] = [
                "from pathlib import Path\n",
                "\n",
                "# Force loading daily data since 5min isn't available\n",
                "data_path = project_root / CONFIG['data']['directory'] / 'SPY_daily.csv'\n",
                "assert data_path.exists(), f'Need SPY_daily.csv for SMC notebook'\n",
                "\n",
                "print('5-minute data not found. Using daily data for demonstration.')\n",
        ...
        # Cell that calls detect_mss in a loop - this takes too long on 2500 rows
            ...
            print(f"Fixed MSS detection cell {i}")
