...
class Hammer(BasePattern):
    ...
    def _check_confirmation(
        ...
        if trend == 'down':
            # Confirm with close above hammer high
            if next_close > candle['high'] * (1 + self.confirmation_filter):
                return True, SignalDirection.LONG
            return False, None

        # Hanging Man after uptrend = bearish reversal
        elif trend == 'up':
            # Confirm with close below hanging man low
            if next_close < candle['low'] * (1 - self.confirmation_filter):
                return True, SignalDirection.SHORT
            return False, None

        # Sideways - use next candle direction
        else:
            if next_close > candle['high']:
                return True, SignalDirection.LONG
            elif next_close < candle['low']:
                return True, SignalDirection.SHORT
            return False, None
    ...
    def detect(
        ...
        signal = TradeSignal(
            ...
            metadata={
                ...
                'lower_shadow_body_ratio': candle['lower_shadow'] / candle['body'] if candle['body'] > 0 else 0,
            },
        )
