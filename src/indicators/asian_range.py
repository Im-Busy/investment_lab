...
def detect_asian_range(
    ...
    # Log warning if bar count is not expected (96 for 5-min bars over 8 hours)
    ...
    # Use the last available ATR value from before the session
    try:
        ...
        atr_value = atr_series.loc[session_end_idx] if session_end_idx in atr_series.index else atr_series.iloc[-1]
...
def calculate_session_statistics(
    ...
    for date in dates:
        ...
        atr_value = atr_series.loc[session_end_time] if session_end_time in atr_series.index else np.nan
