    ...
    if hasattr(signal, "direction"):
        if signal.direction == SignalDirection.LONG:
            if signal.stop_loss >= signal.entry_price:
                errors.append(
                    f"Long position: Stop loss ({signal.stop_loss}) must be below entry ({signal.entry_price})"
                )
        elif signal.direction == SignalDirection.SHORT:
            if signal.stop_loss <= signal.entry_price:
                errors.append(
                    f"Short position: Stop loss ({signal.stop_loss}) must be above entry ({signal.entry_price})"
                )
...
def validate_pattern_params(pattern_name: str, params: Dict[str, Any]) -> tuple:
    """
    ...
    if pattern_name == "DoubleTop" or pattern_name == "DoubleBottom":
        if "peak_tolerance" in params:
            if not isinstance(params["peak_tolerance"], (int, float)) or not (
                0 <= params["peak_tolerance"] <= 1
            ):
    ...
    if pattern_name == "TripleTop":
        if "peak_tolerance" in params:
            if not isinstance(params["peak_tolerance"], (int, float)) or not (
                0 <= params["peak_tolerance"] <= 0.1
            ):
                errors.append(
                    f"peak_tolerance for TripleTop should be <= 0.1 (10%): {params['peak_tolerance']}"
                )

    if pattern_name == "DeadCatBounce":
        if "event_decline_pct" in params:
            if (
                not isinstance(params["event_decline_pct"], (int, float))
                or params["event_decline_pct"] < 0
            ):
                errors.append(
                    f"event_decline_pct must be non-negative: {params['event_decline_pct']}"
                )

    if pattern_name == "Gartley":
        if "xab_tolerance" in params:
            if not isinstance(params["xab_tolerance"], (int, float)) or not (
                0 <= params["xab_tolerance"] <= 0.1
            ):
...
def validate_backtest_config(config: Dict[str, Any]) -> tuple:
    ...
    if "commission_per_trade" in config:
        if (
            not isinstance(config["commission_per_trade"], (int, float))
            or config["commission_per_trade"] < 0
        ):
            errors.append(
                f"commission_per_trade must be non-negative: {config['commission_per_trade']}"
            )

    if "commission_pct" in config:
        if not isinstance(config["commission_pct"], (int, float)) or not (
            0 <= config["commission_pct"] <= 1
        ):
            errors.append(f"commission_pct must be between 0 and 1: {config['commission_pct']}")

    # Slippage validation
    if "slippage_pct" in config:
        if not isinstance(config["slippage_pct"], (int, float)) or not (
            0 <= config["slippage_pct"] <= 0.1
        ):
            errors.append(f"slippage_pct must be between 0 and 0.1: {config['slippage_pct']}")

    # Risk validation
    if "risk_per_trade" in config:
        if not isinstance(config["risk_per_trade"], (int, float)) or not (
            0 < config["risk_per_trade"] <= 0.1
        ):
    ...
    if "max_open_positions" in config:
        if not isinstance(config["max_open_positions"], int) or config["max_open_positions"] < 1:
            errors.append(
                f"max_open_positions must be a positive integer: {config['max_open_positions']}"
            )

    # Confidence threshold
    if "min_confidence" in config:
        if not isinstance(config["min_confidence"], (int, float)) or not (
...
def validate_date_range(start_date: str, end_date: str, df: Optional[pd.DataFrame] = None) -> tuple:
    """
