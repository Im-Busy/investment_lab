...
from src.strategies.confluence import ConfluenceScorer, ConfluenceScore
...
class MultiPatternStrategy(Strategy):
    ...
    def init(self):
        ...
        self.confluence_scorer = ConfluenceScorer(
            ...
            regime_adaptation=self.use_regime_filter
        )
    ...
    def next(self):
        ...
        if len(long_signals) >= len(short_signals) and len(long_signals) >= self.min_confluence_count:
        ...
        if total_confidence > 0:
            stop_loss = sum(s['stop_loss'] * s['confidence'] for s in active_signals) / total_confidence
        else:
