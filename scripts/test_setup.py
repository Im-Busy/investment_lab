...
def verify_atr_calculation() -> Dict[str, Any]:  # type: ignore[return-value]
    ...
    try:
        ...
        if max_deviation < 20:  # Allow 20% difference between methods
            result['passed'] = True
            result['message'] = f"ATR methods differ by max {max_deviation:.2f}% (EMA vs SMA is expected)"
        else:
    ...
    try:
        ...
        if validators_path.exists():
            ...
            try:
                ...
                result['tests']['validators_import'] = {
                    'success': False,
                    'error': str(e)
                }
        else:
...
def verify_project_structure() -> Dict[str, Any]:  # type: ignore[return-value]
    ...
    if not critical_missing:
        result['passed'] = True
        result['message'] = f"Project structure OK ({len(existing_dirs)}/{len(required_dirs)} directories)"
    else:
