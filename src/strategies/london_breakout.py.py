...
class LondonBreakoutParams:
    ...
    tokyo_start_hour: int = 2
    tokyo_end_hour: int = 3
    london_trading_minutes: int = 30
    london_close_hour: int = 12
...
class LondonBreakoutStrategy(Strategy):
    ...
    def _is_london_session_start(self, dt: pd.Timestamp) -> bool:
        ...
        return dt.hour == self.params.tokyo_end_hour and dt.minute == 0

    def _is_london_trading_window(self, dt: pd.Timestamp) -> bool:
        """Check if within London trading window."""
        return (
            dt.hour == self.params.tokyo_end_hour and dt.minute < self.params.london_trading_minutes
        )
    ...
    def _set_london_thresholds(self) -> None:
        ...
        self.upper_threshold = self.london_open_price + self.params.param * self.tokyo_range
        self.lower_threshold = self.london_open_price - self.params.param * self.tokyo_range
