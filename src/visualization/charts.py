...
class ChartGenerator:
    ...
    def plot_with_patterns(
        ...
        if not marker_series.empty:
            addplots.append(
                mpf.make_addplot(
                    marker_series, type="scatter", markersize=100, marker="^", color="green"
                )
            )
        ...
        if not sl_series.empty:
            addplots.append(
                mpf.make_addplot(sl_series, type="scatter", markersize=50, marker="x", color="red")
            )
        ...
        for name, series in tp_series.items():
            if not series.empty:
                addplots.append(
                    mpf.make_addplot(
                        series, type="scatter", markersize=50, marker="o", color="blue"
                    )
                )
    ...
    def plot_with_trades(
        ...
        if not entry_markers.dropna().empty:
            addplots.append(
                mpf.make_addplot(
                    entry_markers, type="scatter", markersize=120, marker="^", color="lime"
                )
            )
        ...
        if not exit_markers.dropna().empty:
            addplots.append(
                mpf.make_addplot(
                    exit_markers, type="scatter", markersize=120, marker="v", color="red"
                )
            )
    ...
    def plot_equity_curve(
        ...
        if benchmark is not None:
            ax.plot(
                benchmark.index,
                benchmark.values,
                label="Benchmark",  # type: ignore[arg-type]
                linewidth=1.5,
                alpha=0.7,
                linestyle="--",
            )
    ...
    def _create_signal_markers(self, df: pd.DataFrame, signals: List[Dict[str, Any]]) -> pd.Series:
    ...
    def _create_stop_loss_markers(
        self, df: pd.DataFrame, signals: List[Dict[str, Any]]
    ) -> pd.Series:
    ...
    def _add_legend(self, fig: Any, ax: Any, signals: List[Dict[str, Any]]) -> None:
