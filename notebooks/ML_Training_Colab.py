...
# Download data
df = yf.download(
    CONFIG["symbol"], start=CONFIG["start_date"], end=CONFIG["end_date"], progress=True
)
    ...
    features["bullish_engulfing"] = (
        ...
        .apply(
            lambda x: (
                x.iloc[0] == False
                and x.iloc[1] == True
                and df["Close"].iloc[1] > df["Open"].iloc[0]
                and df["Close"].iloc[0] < df["Open"].iloc[1]
            )
        )
    ).astype(int)
...
sns.heatmap(
    cm, annot=True, fmt="d", cmap="Blues", xticklabels=["Down", "Up"], yticklabels=["Down", "Up"]
)
...
importance = pd.DataFrame(
    {"feature": feature_columns, "importance": model.feature_importance(importance_type="gain")}
)
...
print("Google Drive folder: /content/drive/MyDrive/ML_Models/")
