...
from .engine import BacktestConfig as HomemadeConfig
from .engine import BacktestEngine
from .engine import BacktestResult as HomemadeResult
...
class UnifiedBacktestConfig:
    ...
    def to_backtesting_py_params(self) -> Dict[str, Any]:
        ...
        params["initial_cash"] = self.initial_equity
        params["commission"] = self.commission_pct
        params["min_confidence"] = self.min_confidence
        params["min_confluence_count"] = self.min_confluence_count

        # Add pattern filters if specified
        if self.include_patterns:
            params["include_patterns_only"] = ",".join(cast(List[str], self.include_patterns))  # type: ignore[assignment]
        if self.exclude_patterns:
            params["exclude_patterns"] = ",".join(cast(List[str], self.exclude_patterns))  # type: ignore[assignment]
...
class UnifiedBacktestResult:
    ...
    @classmethod
    def from_homemade(
        cls, result: HomemadeResult, config: UnifiedBacktestConfig
    ...
    def from_backtesting_py(
        cls, results: Dict[str, Any], config: UnifiedBacktestConfig
    ) -> "UnifiedBacktestResult":
        ...
        if trades_raw is not None:
            trades = (
                trades_raw if isinstance(trades_raw, pd.DataFrame) else pd.DataFrame(trades_raw)
            )
        else:
            trades = pd.DataFrame()
    ...
    def compare_engines(
        ...
        if patterns is not None:
            try:
                ...
                results["homemade_error"] = str(e)  # type: ignore[assignment]
        ...
        if BACKTESTING_AVAILABLE:
            try:
                ...
                results["backtesting.py_error"] = str(e)  # type: ignore[assignment]

        # Compare metrics if both succeeded
        if "homemade" in results and "backtesting.py" in results:
            comparison = self._compare_results(results["homemade"], results["backtesting.py"])
            results["comparison"] = comparison  # type: ignore[assignment]
