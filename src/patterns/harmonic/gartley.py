...
class GartleyPattern(BasePattern):
    ...
    def detect_vectorized(self, df: pd.DataFrame) -> np.ndarray:
        """
        Vectorized Gartley pattern detection across all bars.
        ...
        for i in range(min_bars, n):
            ...
            ls = sorted(lows, key=lambda x: x[0])

            # Bullish Gartley: X=Low, A=High, B=Low, C=High, D=Low
            if len(ls) >= 3 and len(hs) >= 2:
                D = ls[-1]
                C_cand = [(idx, price) for idx, price in hs if idx < D[0]]
                if C_cand:
                    C = C_cand[-1]
                    B_cand = [(idx, price) for idx, price in ls if idx < C[0]]
                    if B_cand:
                        B = B_cand[-1]
                        A_cand = [(idx, price) for idx, price in hs if idx < B[0]]
                        if A_cand:
                            A = A_cand[-1]
                            X_cand = [(idx, price) for idx, price in ls if idx < A[0]]
                            if X_cand:
                                X = X_cand[-1]
                                if self._validate_fib_ratios(X[1], A[1], B[1], C[1], D[1]):
                                    if D[0] == i or i > D[0]:
                                        signals[i] = 1
            ...
            if len(hs) >= 3 and len(ls) >= 2:
                D = hs[-1]
                C_cand = [(idx, price) for idx, price in ls if idx < D[0]]
                if C_cand:
                    C = C_cand[-1]
                    B_cand = [(idx, price) for idx, price in hs if idx < C[0]]
                    if B_cand:
                        B = B_cand[-1]
                        A_cand = [(idx, price) for idx, price in ls if idx < B[0]]
                        if A_cand:
                            A = A_cand[-1]
                            X_cand = [(idx, price) for idx, price in hs if idx < A[0]]
                            if X_cand:
                                X = X_cand[-1]
                                if self._validate_fib_ratios(
                                    X[1], A[1], B[1], C[1], D[1], bearish=True
                                ):
                                    if D[0] == i or i > D[0]:
                                        signals[i] = -1
    ...
    def _identify_bullish_pivots(
        self, highs: List[Tuple[int, float]], lows: List[Tuple[int, float]], current_idx: int
    ...
    def _validate_fib_ratios(
        self, X: float, A: float, B: float, C: float, D: float, bearish: bool = False
    ) -> bool:
        """
        ...
        if not (
            self.BC_RETRACEMENT_MIN - self.fib_tolerance
            <= bc_ratio
            <= self.BC_RETRACEMENT_MAX + self.fib_tolerance
        ):
            return False

        # Validate CD = 1.27 to 1.618 BC
        if not (
            self.CD_EXTENSION_MIN - self.fib_tolerance
            <= cd_ratio
            <= self.CD_EXTENSION_MAX + self.fib_tolerance
        ):
            return False
...
class ButterflyPattern(BasePattern):
    ...
    def _validate_fib_ratios(self, X, A, B, C, D, bearish=False):
        ...
        if not (
            self.XD_RETRACEMENT_MIN - self.fib_tolerance
            <= xd_ratio
            <= self.XD_RETRACEMENT_MAX + self.fib_tolerance
        ):
            return False
    ...
    def _generate_signal(self, df, i, X, A, B, C, D, direction):
        ...
        if direction == "bullish":
            entry_bar_high = (
                self._safe_float(df.iloc[D[0] + 1]["High"])
                if self.require_confirmation and D[0] + 1 <= i
                else D[1]
            )
            entry_price = entry_bar_high + self.entry_offset
            ...
            entry_bar_low = (
                self._safe_float(df.iloc[D[0] + 1]["Low"])
                if self.require_confirmation and D[0] + 1 <= i
                else D[1]
            )
            entry_price = entry_bar_low - self.entry_offset
...
class BatPattern(BasePattern):
    ...
    def __init__(
        ...
        super().__init__(
            name="Bat Pattern", pattern_type=PatternType.REVERSAL, min_bars_required=lookback * 6
        )
        self.lookback = lookback
        self.entry_offset = entry_offset
        self.stop_offset = stop_offset
    ...
    def _validate_fib_ratios(self, X, A, B, C, D, bearish=False):
        ...
        # AB = 0.382-0.5 XA
        if not (
            self.AB_RETRACEMENT_MIN - self.fib_tolerance
            <= ab_ratio
            <= self.AB_RETRACEMENT_MAX + self.fib_tolerance
        ):
            return False
