...
class PatternMarkerGenerator:
    ...
    def generate_markers(
        ...
        for signal in signals:
            ...
            if entry_price is not None:
                self.markers.append(PatternMarker(
                    timestamp=timestamp,
                    price=entry_price,
            ...
            if marker.timestamp in df.index:
                if marker.marker_type == MarkerType.ENTRY:
                    if marker.direction == 'LONG':
                        result['entry_long'][marker.timestamp] = marker.price
                    else:
                        result['entry_short'][marker.timestamp] = marker.price
                elif marker.marker_type == MarkerType.STOP_LOSS:
                    result['stop_loss'][marker.timestamp] = marker.price
                elif marker.marker_type in [MarkerType.TAKE_PROFIT_1, 
                                            MarkerType.TAKE_PROFIT_2,
                                            MarkerType.TAKE_PROFIT_3]:
                    result['take_profit'][marker.timestamp] = marker.price
                elif marker.marker_type == MarkerType.EXIT:
                    result['exit'][marker.timestamp] = marker.price
    ...
    def get_marker_style(
        ...
        """
        direction_style = self.DIRECTION_STYLES.get(
            marker.direction, 
            self.DIRECTION_STYLES['LONG']
        )
        type_style = self.TYPE_STYLES.get(
            marker.marker_type,
            self.TYPE_STYLES[MarkerType.ENTRY]
        )
