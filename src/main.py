...
from .patterns.base import BasePattern, PatternType, SignalDirection
from .patterns.basic import (
    MarketStructureLow,
    MatchingLows,
    NR7ID,
    NBarDecline,
...
from .signals import SignalGenerator, PositionManager
from .backtest import BacktestEngine, PerformanceMetrics, BacktestAdapter, UnifiedBacktestConfig, BacktestEngineType
...
class PatternDetectionSystem:
    ...
    def __init__(
        ...
        logger.info(f"Initialized Pattern Detection System with {len(self.patterns)} patterns (engine: {engine})")
    ...
    def run_backtest(
        ...
        logger.info(f"Backtest complete: {total_trades} trades (engine: {result.engine_type.value})")
    ...
    def get_pattern_statistics(self) -> pd.DataFrame:
        ...
        return PerformanceMetrics.calculate_pattern_statistics(self.last_results.trades)
...
Examples:
  ...
  python -m src.main backtest data/SPY_historical.csv --output results.json
    ...
    backtest_parser.add_argument('--confidence', type=float, default=0.5, help='Min confidence threshold')
    backtest_parser.add_argument('--category', choices=['basic', 'harmonic', 'complex', 'classic', 'all'],
                                default='all', help='Pattern category to use')
    ...
    backtest_parser.add_argument('--engine', choices=['backtesting.py', 'homemade', 'auto'],
                                default='backtesting.py', help='Backtest engine to use (default: backtesting.py)')
    ...
    scan_parser.add_argument('--category', choices=['basic', 'harmonic', 'complex', 'classic', 'all'],
                            default='all', help='Pattern category to use')
    scan_parser.add_argument('--confidence', type=float, default=0.5, help='Min confidence threshold')
    ...
    viz_parser.add_argument('--tearsheet', action='store_true', help='Generate quantstats tearsheet')
    viz_parser.add_argument('--chart', action='store_true', help='Generate price chart with patterns')
    viz_parser.add_argument('--all', action='store_true', help='Generate all reports')
    
    if args.command == 'visualize':
        from .visualization import ReportGenerator, ChartGenerator
    ...
    if args.command == 'smc':
        from .strategies import SMCReversalStrategy, SMCConfig
        ...
        if args.report:
            ...
            files = report_gen.generate_full_report(
                results=result.get_visualization_data(),
                df=df,
                title='SMC Strategy'
            )
            print("\nGenerated reports:")
