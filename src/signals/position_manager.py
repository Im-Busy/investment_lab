...
class PositionManager:
    ...
    def check_exit(
        ...
        if position.direction == SignalDirection.LONG:
            ...
            if position.take_profit_1 and current_high >= position.take_profit_1:
                return True, "Take Profit 1", position.take_profit_1
            if position.take_profit_2 and current_high >= position.take_profit_2:
                return True, "Take Profit 2", position.take_profit_2
            if position.take_profit_3 and current_high >= position.take_profit_3:
            ...
            # Check take profit levels
            if position.take_profit_1 and current_low <= position.take_profit_1:
                return True, "Take Profit 1", position.take_profit_1
            if position.take_profit_2 and current_low <= position.take_profit_2:
                return True, "Take Profit 2", position.take_profit_2
            if position.take_profit_3 and current_low <= position.take_profit_3:
                return True, "Take Profit 3", position.take_profit_3
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        total_wins = sum(p.pnl for p in self.positions.values() 
                        if p.status == PositionStatus.CLOSED and p.pnl and p.pnl > 0)
        total_losses = abs(sum(p.pnl for p in self.positions.values() 
                              if p.status == PositionStatus.CLOSED and p.pnl and p.pnl < 0))
