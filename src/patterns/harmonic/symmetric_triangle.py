...
class SymmetricTriangle(BasePattern):
    ...
    def _calculate_trendline_slope(
        self,
    ...
    def _calculate_triangle_depth(
        ...
        max_high = max([h[1] for h in lower_highs])
        min_low = min([l[1] for l in higher_lows])
    ...
    def _check_volatility_contraction(
        ...
        avg_pattern_range = np.mean(pattern_ranges)
        ...
        if end_idx < len(avg_range):
            ...
            return avg_pattern_range < overall_avg
    ...
    def detect(self, df: pd.DataFrame, i: int) -> PatternResult:
        ...
        volatility_contracting = self._check_volatility_contraction(df, start_idx, end_idx)
    ...
    def _generate_signal(
        ...
        swing_low = min([l[1] for l in higher_lows])
        ...
        pattern_bars = max(lower_highs[-1][0], higher_lows[-1][0]) - min(lower_highs[0][0], higher_lows[0][0])
        if pattern_bars > 0:
