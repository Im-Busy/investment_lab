...
from ...indicators.technical import bollinger_bands, atr, volume_sma, sma
...
class BollingerBands(BasePattern):
    ...
    def _is_squeeze(self, bands: Dict) -> bool:
        ...
        return squeeze_ratio < self.squeeze_threshold
    ...
    def detect(self, df: pd.DataFrame, i: int) -> PatternResult:
        ...
        return PatternResult(
            detected=True,
            pattern_name=f"{self.name} ({pattern_type.replace('_', ' ').title()})",
            pattern_type=self.pattern_type,
            signal=signal,
            pivot_points={
                **bands,
...
class BollingerSqueeze(BasePattern):
    ...
    def detect(self, df: pd.DataFrame, i: int) -> PatternResult:
        ...
        bandwidth = self._safe_float(bands['bandwidth'].iloc[i])
        avg_bandwidth = self._safe_float(bands['bandwidth'].rolling(window=self.lookback).mean().iloc[i])
