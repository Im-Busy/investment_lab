...
def fetch_daily_data(ticker: str, start: str, end: str, output_dir: str = "data/raw") -> bool:
    """
    ...
    try:
        ...
        if not isinstance(df, pd.DataFrame) or df.empty:  # type: ignore[union-attr]
...
def fetch_intraday_data(
    ticker: str, period: str = "730d", interval: str = "1h", output_dir: str = "data/raw"
) -> bool:
    """
...
def fetch_all_daily(
    instruments: Optional[Dict] = None, output_dir: str = "data/raw"
) -> Dict[str, bool]:
    """
    ...
    for ticker, config in instruments.items():
        success = fetch_daily_data(
            ticker=ticker, start=config["start"], end=config["end"], output_dir=output_dir
        )
