...
def test_strategies():
    ...
    print("=" * 100)
    df = pd.read_csv(
        project_root / "data" / "raw" / "BTC_USD_1h.csv", index_col="Datetime", parse_dates=True
    )
    ...
    for name, mod_path, cls_name in STRATEGY_IMPORTS:
        try:
            ...
            print(
                f"  {name:<25} t={r['trades']:>5} sh={r['sharpe']:>7.2f} wr={r['wr']:>6.1f}% ret={r['return_pct']:>7.1f}%"
            )
...
def optimize_pattern(aname, df, pat):
    ...
    try:
        bt = Backtest(
            df, MultiPatternStrategyOptimized, cash=1000000, commission=0.001, exclusive_orders=True
        )
        opt = bt.optimize(
...
if __name__ == "__main__":
    ...
    with open(od / "summary.md", "w") as f:
        ...
        for r in strat_results:
            if r.get("ok"):
                f.write(
                    f"| {r['name']} | {r['trades']} | {r['wr']:.1f} | {r['sharpe']:.2f} | {r['return_pct']:.1f} |\n"
                )
        f.write("\n## Optimization (SPY & QQQ)\n\n")
        f.write(
            "| Asset | Pattern | Sharpe | PF | Return% | DD% |\n|-------|---------|--------|----|----------|-----|\n"
        )
        for r in opt_results:
            if "error" not in r:
                f.write(
                    f"| {r['asset']} | {r['pat']} | {r['sharpe']:.2f} | {r['pf']:.2f} | {r['return_pct']:.1f} | {r['dd_pct']:.1f} |\n"
                )
