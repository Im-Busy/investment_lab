...
class ParabolicArc(BasePattern):
    ...
    def detect_vectorized(self, df: pd.DataFrame) -> np.ndarray:
        """
        Vectorized Parabolic Arc detection across all bars.
        ...
        min_bars = self.min_base_bars + self.min_vertical_bars
        ...
        for i in range(min_bars, n):
            ...
            for j in range(max(0, peak_idx - 60), peak_idx):
                if n_sl[j]:
                    base_lows.append((j, float(sl_arr[j])))
            ...
            vert_bars = peak_idx - base_idx
            if vert_bars < self.min_vertical_bars:
                continue

            price_change = peak_price - base_price
            avg_slope = price_change / vert_bars / base_price
            ...
            for j in range(peak_idx + 1, i + 1):
                approach_ratio = abs(float(high[j]) - peak_price) / peak_price
                if approach_ratio < self.failed_test_threshold:
                    tests.append((j, float(high[j])))

            # Trendline break check
            tl_lows: list = []
            for j in range(base_idx, peak_idx):
                if n_sl[j]:
                    tl_lows.append((j, float(sl_arr[j])))
            trendline_break = False
            if len(tl_lows) >= 2:
                fl = tl_lows[0]
                ll = tl_lows[-1]
                tl_slope = (ll[1] - fl[1]) / (ll[0] - fl[0]) if ll[0] != fl[0] else 0.0
                tl_inter = fl[1] - tl_slope * fl[0]
                tl_val = tl_slope * i + tl_inter
                trendline_break = float(close[i]) < tl_val

            failed = len(tests) >= 2 and tests[-1][1] < tests[0][1]

            if failed or trendline_break:
                signals[i] = -1

        return signals

    def _find_parabolic_move(self, df: pd.DataFrame, i: int) -> Optional[Dict]:
        """
    ...
    def _check_failed_test(self, df: pd.DataFrame, i: int, para: Dict) -> Optional[Dict]:
        """
    ...
    def _check_trendline_break(self, df: pd.DataFrame, i: int, para: Dict) -> bool:
        """
        ...
        # Calculate trendline
        slope = (
            (last_low[1] - first_low[1]) / (last_low[0] - first_low[0])
            if last_low[0] != first_low[0]
            else 0
        )
    ...
    def _generate_signal(
        self, df: pd.DataFrame, i: int, para: Dict, failed_test: Optional[Dict]
...
class ParabolicBase(BasePattern):
    ...
    def detect(self, df: pd.DataFrame, i: int, window_start: Optional[int] = None) -> PatternResult:
        ...
        signal = TradeSignal(
            ...
            confidence=0.55,
            timestamp=df.iloc[i].name if hasattr(df.iloc[i], "name") else None,
            metadata={"base_range": base_range, "volatility": volatility, "entry_type": "buy_stop"},
        )
