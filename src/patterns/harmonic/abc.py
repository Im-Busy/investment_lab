...
from ...indicators.fibonacci import calculate_bc_retracement
...
class ABCPattern(BasePattern):
    ...
    def __init__(
        ...
        """
        super().__init__(
            name="ABC Pattern", pattern_type=PatternType.REVERSAL, min_bars_required=lookback * 4
        )
    ...
    def detect_vectorized(self, df: pd.DataFrame) -> np.ndarray:
        """
        Vectorized ABC pattern detection across all bars.
        ...
        for i in range(min_bars, n):
            ...
            # Bullish ABC: A=High, B=Low, C=High (lower high), correction in uptrend
            if len(hs) >= 2 and ls:
                C = hs[-1]
                ...
                if B_cand:
                    ...
                    if A_cand:
                        A = A_cand[-1]
                        if A[1] > B[1] and C[1] > B[1] and C[1] < A[1]:
                            ...
                            if (
                                ...
                                if not self.require_trend or X_cand:
                                    if C[0] == i:
                                        signals[i] = -1

            # Bearish ABC: A=Low, B=High, C=Low (higher low), correction in downtrend
            if len(ls) >= 2 and hs:
                C = ls[-1]
