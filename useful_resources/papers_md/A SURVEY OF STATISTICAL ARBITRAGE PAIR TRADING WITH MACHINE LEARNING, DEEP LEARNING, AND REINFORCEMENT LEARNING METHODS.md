...
> *Source PDF: A SURVEY OF STATISTICAL ARBITRAGE PAIR TRADING WITH MACHINE LEARNING, DEEP LEARNING, AND REINFORCEMENT LEARNING METHODS.pdf*
...
Sun, Y. / WORKING PAPERS 22/2025 (485)
...
how  computational  approaches  have  progressively

transformed  strategy  design  and
...
events)  and  addressing  class

imbalance

in  financial  datasets—which  require  careful
...
CSI 300 index futures (IF)

CSI 300 exchange traded fund (ETF)
...
1027 NSE (National Stock Exchange of India)
...
PCA, DBSCAN
K-Means, DBSCAN, AHC
OPTICS Clustering Algorithm (OPTICS)
EM Algorithm (EM)
LGBM, RF, SVR
...
LSTM
Neural Network
KalmanNet Bollinger Trading (KalmanBOT)
Stochastic Neural Network (SNN)
RNN, LSTM, TCN
TCN
LSTM, LSTM Encoder-Decoder
LSTM, CNN, Multilayer perceptron (MLP)
DNN
BNN, DP-BNN, DDP-BNN
Filterbank CNN
...
CA-DRL, NEWS-CO-DRL
PPO, PPO-PT, PPO-PT w/o Demo, SAPT, PTDQN
Hierarchical Reinforcement Learning (HRL)
P-DDQN, PTDQN
SAPT, SAPT w/o Break, SAPT w/o Time, SAPT w/o Hold, PTDQN, SAPT-3-std,
SAPT-ADF, SAPT-BCD, SAPT-LSTM
RL-0, RL-0.02, RL-0.05, RL-0.1
Deep Reinforcement Learning (DRL)
DQN, Double Deep Q-Network (DDQN)
RL
...
Combined Models
PCA, Convolutional AutoEncoders (CAE), DBSCAN, PPO
LR, XGBoost, CNN, LSTM
A-LSTM+1/N, RF+MVF, RF+1/N, SVM+MVF, SVM+1/N
LSTM, OPTICS
XGBoost, TCAN
SVM, XGBoost, DNN, LSTM, RAF
LSTM, ANN, LR, GBDT, FeedForward NN
DQN, Double Deep Q-Network (DDQN)
LSTM, CNN, Deterministic Policy Gradient (DPG)
ANN, XGBoost
LR, Gaussian Discriminant Analysis (GDA), SVM, NN
DNN, GBDT, RF
...
Parameters
kernel=poly, C=0.1, degree=3, coef0=2
kernel=poly, C=1, degree=3, coef0=0.5
kernel=rbf, C=0.1, gamma=0.01, coef0=0
kernel=poly, C=4, degree=3, coef0=5
Kernel=poly, C=2, degree=2, coef0=2
...
𝐿(𝜃) = 𝐸(𝑠,𝑎,𝑟,𝑠′)∼𝑟𝑒𝑝𝑙𝑎𝑦 𝑏𝑢𝑓𝑓𝑒𝑟 [(𝑟 + 𝛾𝒎𝒂𝒙

𝑎′

𝑄(𝑠′, 𝑎′; 𝜃−) − 𝑄(𝑠, 𝑎; 𝜃))2]      (5.15)
...
errors in a set of predictions, without
considering their direction. It is the
mean over the test sample of the
absolute differences between predicted
values and actual values.
The average squared difference between
the estimated values and the actual
value. MSE is more sensitive to outliers
than MAE as it squares the differences.
RMSE is the square root of the mean of
the squared errors. RMSE is particularly
useful when large errors are particularly
undesirable.

MAPE is a measure of prediction
accuracy of a forecasting method in
statistics. It usually expresses the
accuracy as a ratio.

Measures the overall effectiveness of
the model in predicting correct
outcomes.

Error rate

It quantifies the frequency at which the
model accurately forecasts the outcome.
...
Evaluates the reliability of the model in
predicting positive (converging)
outcomes.
Assesses the model’s capability to
identify actual positive (converging)
instances.
Balances precision and recall,
particularly useful when the cost of false
...
Description
Accumulated Return denotes the overall percentage
gain or declines in the value of an investment
throughout its duration. Ideally, the Accumulated
Return should be positive and maximized.

Annual return is the percentage change in the value
of an investment over a one-year period, reflecting
the compound rate of return earned or lost by the
investment annually.

The Sharpe ratio is a measure used to evaluate the
risk-adjusted return of an investment by comparing
its excess return over the risk-free rate to the
standard deviation of those returns.

Article
[3, 6, 7, 8, 10, 17, 18, 29, 39,
42, 44, 53, 62, 65]

[2, 4, 5, 8, 12, 22, 25, 31, 32,
33, 35, 39, 41, 46, 47, 55, 58,
59, 63, 68]

[2, 3, 4, 5, 6, 8, 9, 10, 12, 13,
14, 15, 18, 20, 21, 22, 26, 29,
31, 33, 34, 38, 39, 40, 41, 43,
44, 45, 47, 49, 50, 55, 58, 62,
65, 66, 68]
...
Description
 Maximum Drawdown is a risk measure that
indicates the largest single drop from peak to trough
in the value of a portfolio or an investment, before a
new peak is achieved.

Article
[2, 3, 5, 6, 8, 9, 10, 12, 15, 18,
20, 21, 22, 25, 26, 27, 29, 31,
33, 35, 38, 39, 41, 43, 45, 47,
58, 61, 64, 68]

Standard Deviation
(Volatility)

quantifies the variability or dispersion of a set of
data points from their mean, typically used to assess
the consistency of an investment's returns.

[2, 4, 5, 8, 12, 33, 36, 38, 40,
41, 55, 58, 61, 62, 63, 65, 66,
68]
...
Refers to the asymmetry in the distribution of returns
...
Measures the excess return of an investment relative
to the downward deviation, focusing specifically on
...
in  some  cases  outperformed—by  advanced
...
Neagu, B.C.,  2022.  Stochastic  neural  networks-based  algorithmic
